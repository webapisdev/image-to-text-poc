...
export default function UploadImage() {
  ...
  const [unsuportedMediaErrorMessage, setUnsuportedMediaErrorMessage] =
    useState("");
  ...
  function handleUpload2(event) {
    ...
    fetch("/api/UploadImage", requestOptions)
      ...
      .catch((error) =>
        setUnsuportedMediaErrorMessage(
          "Unsupported Media Type. Only images are supported at this time"
        )
      );
  }
  return (
        ...
        {!unsuportedMediaErrorMessage ? (
          <Container>
            {Object.keys(imageToText).length > 0 && (
              <ImageTextCard
                imageUrl={imageToText.Url}
                imageText={imageToText.Text}
              />
            )}
          </Container>
        ) : (
